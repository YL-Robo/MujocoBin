...
def plot_trajectories_with_tcp_slerp(file_paths, labels,
                               arrow_interval=5,
                               axis_len=0.05,
                               axes='XYZ'):
    """
    ...
    ax.set_title('Direct vs SLERP Trajectories with TCP Frames in Mujoco', fontsize=14)
    ax.legend()
    set_axes_equal(ax)
    plt.show()

def plot_trajectories_with_tcp_so3(file_paths, labels,
                               arrow_interval=5,
                               axis_len=0.05,
                               axes='XYZ'):
    """
    ...
    file_paths: [direct_path, so3_path]
    labels: ["direct", "so3"]
    arrow_interval: 每隔多少个点画一次姿态
    axis_len: 坐标系箭头长度
    axes: 哪些轴要画，'XYZ'表示都画
    ...
    for idx, (file_path, label) in enumerate(zip(file_paths, labels)):
        traj_data = np.load(file_path, allow_pickle=True)  # [(pos, quat), ...]
        positions = np.array([p for p, _ in traj_data])
        rots= np.array([q for _, q in traj_data])
    ...
    ax.set_ylabel('Y (m)', fontsize=12)
    ax.set_zlabel('Z (m)', fontsize=12)
    ax.set_title('Direct vs SO3 Trajectories with TCP Frames in Mujoco', fontsize=14)
    ax.legend()
    set_axes_equal(ax)
    plt.show()
...
# ---------------- 使用示例 ----------------
# file_paths = [
#     "/home/zxy/MujocoBin/Data/NPY/slerp/direct_trajectory_data.npy",
#     "/home/zxy/MujocoBin/Data/NPY/slerp/slerp_trajectory_data.npy"
# ]
# file_paths = [
#     "/home/zxy/MujocoBin/Data/NPY/slerp/direct_trajectory_data.npy",
#     "/home/zxy/MujocoBin/Data/NPY/slerp/slerp_trajectory_data.npy"
...
file_paths = [
    "/home/zxy/MujocoBin/Data/NPY/so3/direct_trajectory_data.npy",
    "/home/zxy/MujocoBin/Data/NPY/so3/so3_trajectory_data.npy"
]
file_paths = [
    "/home/zxy/MujocoBin/Data/NPY/so3/direct_trajectory_data.npy",
    "/home/zxy/MujocoBin/Data/NPY/so3/so3_trajectory_data.npy"
]
labels = ["direct_trajectory", "so3_trajectory"]

# plot_trajectories_with_tcp_slerp(
#     file_paths,
#     labels,
#     arrow_interval=5,
#     axis_len=0.02,
...
plot_trajectories_with_tcp_so3(
    file_paths,
    labels,
    arrow_interval=5,
    axis_len=0.02,
